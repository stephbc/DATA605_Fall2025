...
- contingency table: shows counts of products in each (Sales quartile × Price quartile) cell
- joint probabilities: probabilities that an product falls in both a Sales and Price quartile
- marginal probabilities: probabilities of each Sales or Price quartile independently
...
# joint probabilities
prop.table(cont_table)

# marginal probabilities
prop.table(rowSums(cont_table)) # by sales
prop.table(colSums(cont_table)) # by price
```
...
Fisher’s Exact Test = exact, safe for small counts, slower for large tables.

Chi-square Test = approximate, works for large tables, may be inaccurate with small expected counts.

```{r}
fisher_test <- fisher.test(cont_table)
fisher_test

chi_test <- chisq.test(cont_table, correct = FALSE)
chi_test
```
...
Based on the quick plot below, Sales has a right skew, so the exponential distribution is fitted on Sales:

```{r}
ggplot(data = retail, aes(x = Sales)) +
  geom_histogram()
...
```{r}
qexp(0.05, rate = fit_exp$estimate["rate"])
qexp(0.95, rate = fit_exp$estimate["rate"])
```
...
data.frame(
  ...
  Lower = c(ci_lower, empirical_lower),
  Upper = c(ci_upper, empirical_upper)
)
```
...
Positive coefficient → predictor increases Inventory_Levels
Negative coefficient → predictor decreases Inventory_Levels
Magnitude → size of effect per unit increase in predictor

R-squared:

Closer to 1 → model explains more variance in Inventory_Levels

Adjusted R-squared accounts for the number of predictors

Residual plots:

Residuals randomly scattered → linear model assumptions likely valid

Patterns (curvature, funnel shape) → potential violations

...
The t is 2.24 days for a max revenue of 5912.09.
...
# dummy interval to find root
t_32 <- uniroot(R_32_prime, interval = c(0.1, 10))$root
t_32

# plug in to original function for max
max_revenue_32 <- R_32(t_32)
max_revenue_32
```
...
The x that maximizes profit is about 1.28 and the max profit is about 2.4. Since flat irons can only be sold in whole units, this max profit is not feasible, the actual value would be slightly lower.
...
integral_36 <- integrate(dNdt_36, lower = 1, upper = 10)$value
print(6530 + integral_36)
```
...
The below returns the coefficients in descending order, so the approx revenue function is:

1 + x + (x^2)/2.

In business contexts, this type of function is simple and quick to implement for estimates on smaller amounts of data and easy to explain.
...
Combining the above (1 + x + (x^2)/2) - (x - (x^2)/2), we get the approximated profit function:

1 + x^2.

The optimal x for each function below is 0. This implies that the product is not profitable at all.

```{r}
# approximate
Pi_42_approx <- function(x) { 1 + x^2 }
x_42_l <- uniroot(Deriv(Pi_42_approx, "x"), c(0, 2))$root
x_42_l

# original nonlinear function
Pi_42 <- function(x) { exp(x) - log(1 + x) }
x_42_nl <- uniroot(Deriv(Pi_42, "x"), c(0, 2))$root
x_42_nl
```
...
This errors since the square root function is not differentiable when x = 0.
...
The below approx function is 1 + 0.5x - 0.125(x^2).

The results show that for small amounts, the Taylor approx overestimates, but as the investment amounts increase, the differences between the actual and approx risks decrease. 

```{r}
F_42b <- function(x) { sqrt(1 + x) }
taylor(F_42b, 0, n = 2)

t_F42b <- function(x) { 1 + 0.5*x - 0.125*(x^2) }

x_42b <- c(0.01, 0.05, 0.1, 0.2, 0.5, 1)

data.frame(
  x = x_42b,
  actual = F_42(x_42b),
  approx = t_F42b(x_42b),
  error = F_42(x_42b) - t_F42b(x_42b)
)
```
...
So we can take, for example, a simple linear return and subtract the approximate risk times a dummy risk aversion parameter of 2.5, for a balance of risk and return. The optimal investment comes to 0.4 units.

```{r}
return_lin <- function(x) { x }
ra <- 2.5
opt_43 <- function(x) { return_lin(x) - ra * t_F42b(x) }

opt_43_deriv <- Deriv(opt_43, "x")

# interval for small investments
x_43 <- uniroot(opt_43_deriv, c(0, 2))$root
...
The expanded cost function is:

1 + p + (p^2)/2

This could be useful in pricing strategy because it simplifies profit maximization and provides clear insights.
...
The differences in the resulting profits are very close. This means that approximating the cost function is sufficient and useful in cases like this.

```{r}
D_43b <- function(p) { 1 - p }
C_43b <- function(p) { exp(p) }

# original nonlinear
Pi_43b_og <- function(p) { p*D_43b(p) - C_43b(p) }

# approx cost Taylor series 
taylor(C_43b, 0, n = 2)
```

```{r}
# approx cost Taylor series 
C_43b_approx <- function(p) { 1 + p + 0.5*p^2 }

# approx profit function
Pi_43b_approx <- function(p) { p*D_43b(p) - C_43b_approx(p) }

# compare
p43_range <-  c(0.01, 0.05, 0.1, 0.2, 0.5, 1)

data.frame(
  p = p43_range,
  actual = Pi_43b_og(p43_range),
  approx = Pi_43b_approx(p43_range)
)
```
...
A pricing strategy here could be to start small and get an optimal price using the simpler polynomial formula to avoid overcharging and reducing profit.
...
This is simpler to compute with than the logarithmic function and when the x is small, remains an accurate approximation.
...
When tested on a set of very small values, the average difference between the approximated values and the actual values is miniscule at 0.05. This means the approximation is useful and accurate. 
...
To solve for 0.5*x^2 - x = G_target, we can use the quadratic formula to get the investment level for a sample target growth of 5%:
...
# Solve quadratic: 
x1 <- (-b + sqrt(b^2 - 4*a*target)) / (2*a)
x2 <- (-b - sqrt(b^2 - 4*a*target)) / (2*a)
...
Using `optim()`, which minimizes a function, we can negate the profit to maximize it.

(x, y, II(x, y)) = (7.30, 0.26, 112.6957)

This means we need about 7.3 units of x or Product A and 0.26 units of y or Product B to maximize profit. 

In a real-world scenario, we would focus on Product A to maximize profit.

```{r}
profit_51 <- function(vars) {
  x <- vars[1]
  y <- vars[2]
  -(30*x - 2*x^2 - 3*x*y + 24*y - 4*y^2)  # negative for maximization
}

# initial guess of 1 each
result_51 <- optim(c(x = 1, y = 1), profit_51)
result_51
```
...
Using the revenue function below, to maximize revenue, X should be priced at 6.15 and Y at 4.31. 

These are competing brands so raising the price of one might affect the sales and profits of the other.

```{r}
revenue_52 <- function(prices) {
  x <- prices[1]
  y <- prices[2]
...
opt_52 <- optim(c(x = 1, y = 1), revenue_52)
opt_52
```
...
Below the optimization function shows that about 102 units should be produced in NY and the remainder, around 98, should be produced in Chicago. The minimized cost should be around 6748.89.
...
opt_53 <- optimize(cost_53, interval = c(0, 200))

# optimal x
x_opt53 <- opt_53$minimum
x_opt53
...
Optimal spending levels are: 16,667 on online ads and 33,332 on TV ads.

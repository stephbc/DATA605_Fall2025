...
To solve for 0.5*x^2 - x = G_target, we can use the quadratic formula to get a target growth:
...
This means we need about 7.3 units of x or Product A and 0.26 units of y or Product B to maximize profit, so we should focus on Product A to maximize profit.
...
result_51 <- optim(c(x = 1, y = 1), profit)
result_51
```

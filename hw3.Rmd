...
The below returns the coefficients in descending order, so the revenue function is 1 + x + (x^2)/2. In business contexts, this type of function is simple and quick to implement for estimates and easy to explain.
...
The cost function is x - (x^2)/2. This is also simpler and easier to interpret.
...
```{r}
Pi_43 <- function(x) { exp(x) - log(1 + x) }

taylor(Pi_43, 0, n = 2)

Pi_43_exp <- function(x) { }
```
